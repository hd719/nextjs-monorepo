...
- [~] Task: Align response fields with existing FoodItem schema (per-100g values).
  ...
  - [ ] Subtask: Add missing fields to response (`servingSizeG`).
- [~] Task: Confirm request and response headers.
  ...
  - [~] Subtask: Echo `X-Request-ID` from TS server (do not generate new IDs).
- [~] Task: Confirm error codes and mapping to HTTP status.
  - [x] Subtask: INVALID_BARCODE -> 400, NOT_FOUND -> 404, UPSTREAM_ERROR -> 502.
  - [ ] Subtask: Define INTERNAL_ERROR mapping and when it is returned.
  - [ ] Subtask: Ensure error response includes `requestId` in body (per PRD).
...
- [~] Task: Choose entrypoint location (root main.go or cmd/api/main.go).
  ...
  - [x] Subtask: Internal packages started (`ratelimiter`, `internal/barcode`).
...
- [~] Task: Define route group for /v1.
  ...
  - [ ] Subtask: Align router choice with PRD (chi vs gin).
...
- [~] Task: Create response DTOs for barcode lookup.
  ...
  - [ ] Subtask: Add `servingSizeG` if parsing is implemented.
...
- [~] Task: Create error response envelope.
  ...
  - [ ] Subtask: Include `requestId` in error body (per PRD).
  - [ ] Subtask: Add a helper to write error responses consistently.

### Story 2.3: Request ID handling

- [x] Task: Generate request ID if missing.
  - [x] Subtask: Accept X-Request-Id from caller if present.
  - [~] Subtask: Attach requestId to response header (echo only, no generation per PRD).
...
- [~] Task: Create HTTP client with base URL and timeout.
  ...
  - [ ] Subtask: Add timeout and user-agent header for upstream requests.
...
- [~] Task: Add structured request logs.
  - [~] Subtask: Basic request logs via Gin default logger (PRD references chi).
...
- [ ] Task: Accept user JWT forwarded by TS server.
  - [ ] Subtask: Validate `X-API-Key` against `BARCODE_SERVICE_API_KEY`.
  - [ ] Subtask: Verify JWT from `Cookie` using `BETTER_AUTH_SECRET`.
  - [ ] Subtask: Read `X-User-ID` and `X-Request-ID` for auditing/logging.
